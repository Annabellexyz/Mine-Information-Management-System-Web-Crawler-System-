...
<title>深圳电信160:各大厦公司列表搜索结果 </title>
</head>
<?php
// 处理分页
...
$pageNumberSql = "SELECT COUNT(1) AS PageNum FROM `edifice_credit` {$sqlWhere}";
...
// 分页字符串
$pageationedifice_credit = "共{$row['PageNum']}条数据 " . index_page($row['PageNum'], $linksize, $pagesize, 'pageball', array('searchterm', 'searchtype'));

//取当前页数据
$pageball = intval($_GET['pageball']);
$offset = $pagesize*($pageball - 1);

$query ="SELECT * FROM `edifice_credit` {$sqlWhere} ORDER BY id ASC LIMIT $offset,$pagesize";
$result = mysql_query($query);

$contentedifice_credit = '';
// 表格头
	$contentedifice_credit .= 	"<h4 align='CENTER'>深圳企业信息搜索结果</h4>"
...
{
	$contentedifice_credit .= "<tr>"
	. "<td width=6%>{$row['id']}</td>"
	. "<td width=15%>{$row['edifice']}</td>"
	. "<td width=30%>{$row['name']}</td>"
	. "<td width=8%>{$row['person']}</td>"
	. "<td width=23%>{$row['types']}</td>"
	. "<td width=9%>{$row['detail']}</td>"
	. "<td width=9%>{$row['changes']}</td>"
	. "</tr>";
} 
?>
...
<!-- edifice_credit表的记录及分页 -->
<table id="results" width="100%">
<tr><td align="left">
<?php echo $contentedifice_credit ?>
<td><tr>
</table>

<table width="100%">
<tr><td align="right">
<?php echo $pageationedifice_credit ?>
<td><tr>
</table>
<!-- edifice_credit表的记录及分页 -->
</body>
