...
$edittime = date("Y-m-d H:i:s");
     ...
     if(intval($lmbs) > 0){
       ...
       or die(mysql_error());
        ...
        if($query==true)
        { 
         echo "<script>alert('160餐饮资料更新成功!!');window.location.href='/mine/telecom_restaurant_update/index_online.php';</script>";
        }
        
        else{echo "160餐饮资料更新失败!!";}
        echo "<pre>"; print_r($__POST); echo "</pre>";
      }
      else 
      {
      	echo "<font color = red>你没有登录系统没有操作权限！</font>
      	<a href='/mine/telecom_restaurant_update/restaurant_edit.php?searchterm=".$lmbs."&id=".$lmbs."'>请点击此处登录后继续进行编辑！</a>";
        echo "<pre>"; print_r($__POST); echo "</pre>";
      }
}
?>
