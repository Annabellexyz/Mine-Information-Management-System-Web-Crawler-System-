...
{
$progbar->step();
//$company_info='\\172.20.1.21\g$\3'.$cid.'.htm'; //商家资料 修改
  ...
  //echo $name;echo "<br>";
   ...
   $telephone = substr($result,0,9);$telephone=str_replace(" ","",$telephone);//echo "电话：";echo $telephone;echo "<br>";
   	if (($pos4 = strpos($result,'传真：')) != false) { 		 	
	 	...
	 	$fax='0';
 		if (($pos8 = strpos($result,'所属类别')) != false) { 
	  $address = substr($result,9,$pos8-9);//echo "地址：";	echo $address;echo "<br>"; 	
	 }
	  else{
...
/******$query=$db->query("select * from hongkong_page where (name='$name' and telephone='$telephone' and 
category2='$category2' and category3='$category3')");
  if(!($db->num_rows($query)))
  {//检查重复的数据
echo "select * from hongkong_page where (name='$name' and telephone='$telephone' and category2='$category2' and category3='$category3')";
exit();
    }*******/
