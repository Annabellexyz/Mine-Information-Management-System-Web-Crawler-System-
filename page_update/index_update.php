...
$envelopeid = isset($__POST['envelopeid']) ? trim($__POST['envelopeid']) : '';
$remark = isset($__POST['remark']) ? trim($__POST['remark']) : '';
$amount = isset($__POST['amount']) ? trim($__POST['amount']) : '';
...
$qqmsn  = isset($__POST['qqmsn']) ? trim($__POST['qqmsn']) : '';
...
$edittime = date("Y-m-d H:i:s");
     ...
     if(intval($lmbs) > 0){
     	if (!empty($editor)) {
        $pid=$__POST[pid];
        $query=mysql_query("update china_szpage set envelopeid='$envelopeid',remark='$remark',info_code='$info_code',name='$name',fullname='$fullname',
        amount='$amount',
        address='$address',district='$district', road='$road',number='$number',
        edifice='$edifice',floor='$floor',linkman='$linkman', 
        business='$business',telephone='$telephone',new_phone='$new_phone',mobilephone='$mobilephone',fax='$fax',
        website='$website',email='$email',qqmsn='$qqmsn',
        principal='$principal',editor='$editor',       
