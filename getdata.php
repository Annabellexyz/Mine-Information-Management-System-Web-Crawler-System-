...
//以下5行代码删除重复记录，有重复记录的时候再调出来*/
	$query=$db->query("SELECT *,min(id) as ids FROM `myself_matches` WHERE 1 group by day,typename,hometeam,points,
  guestteam,halfpoints HAVING count(*)>1 order by count(*) desc");
	while($record=$db->fetch_array($query))
	{
	    $db->query("delete from myself_matches where day='".$record['day']."' and typename='".$record['typename']."'
