<HTML>
<HEAD>
<TITLE>采集深圳信用网企业信息5301-5898</TITLE>
</HEAD>
<BODY>
<?php
error_reporting();
...
foreach ($strArray as $k => $v){
 ...
 if (!empty($companyArray[1])) 
 {
 	...
 	echo $changes;echo "<br>";echo "<br>";*/
 	    ...
 	    $query=$db->query("select * from credit_page3 where (name='$name' and person='$person' and types='$types')");
   if(!($db->num_rows($query)))
   {
     $query=$db->query("insert into credit_page3(name,person,types,detail,changes) 
