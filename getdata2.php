...
<BODY>
	...
	&nbsp;   <A href="http://172.20.1.21/mine/getdata.php?par=delay"> <U><FONT color=#0000ff>第二步：点击采集最近足球延期数据</FONT></U></A><BR>
	 &nbsp;   <A href="http://172.20.1.21/mine/change.php"> <U><FONT color=#0000ff>第三步：点击更新球队资料</FONT></U></A><BR>
	      ...
	      &nbsp;&nbsp;<font color=red>如果发现有重复记录，则从第一步点击采集最新足球完场赛事数据开始运行。</font><BR>
...
if($par=='day')         //采集某天的所有比赛场次
{
  ...
  $lastday=mktime(0,0,0,6,24,2008);
...
//			{	
				...
				$query=$db->query("select * from myself_matches where day='$m_days[$k]' and time='$m_times[$k]' and hometeam='$ht_names[$k]' and halfpoints='$m_halfpoints[$k]'");
...
//以下5行代码删除重复记录，有重复记录的时候再调出来*/
	$query=$db->query("SELECT *,min(id) as ids FROM `myself_matches` WHERE 1 group by day,time,typename,hometeam,
  guestteam HAVING count(*)>1 order by count(*) desc");
	while($record=$db->fetch_array($query))
	{
	    $db->query("delete from myself_matches where day='".$record['day']."' and time='".$record['time']."'
	    and typename='".$record['typename']."' and hometeam='".$record['hometeam']."' and guestteam='".$record['guestteam']."'
	    and id != '".$record['ids']."'"); 
	}

	//以下7行代码一次删除一条重复记录
	/*$query=$db->query("SELECT min(id) as id FROM `myself_matches` WHERE 1 group by day,typename,hometeam,points,guestteam,halfpoints HAVING count(*)>1 order by count(*) desc");
	while($record=$db->fetch_array($query))
	{
	$del[] = "'".$record['id']."'";
	}
	$del = join($del,",");
	$db->query("delete from myself_matches where id in (".$del.")"); */
